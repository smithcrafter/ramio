...
## 0.1.7b3 RMStringList changes [08.01.2023]
- Добавлена сериализация, сохранение в БД, форма редактирование для StringList
Ограничение: список собирается с разделителем "перенос строки", не доступен в элементах
...
- Реализована структура EPDataObjectsChanged для уведомления изменения нескольких элементов

## 0.1.7a17 scripts [12.08.2022]
- Изменены скрипты сборки (добавлен скрипт без очистки проекта)
...
## 0.1.7a12 DatabaseConnection exec sql [03.06.2022]
- Предоставлен интерфейс для выполнения произвольного sql запроса и обработки его результата.

## 0.1.7a11 containerId [03.06.2022]
- Введён идентификатор контейнера для внешних нужд идентификации
...
## 0.1.7a5 --profile [21.05.2022]
- Аргумент командной строки для изменения директории хранения настроек
...
## 0.1.6h6 Description::property [08.04.2022]
- Добавлена функция поиска свойства по имени в метаописание
...
## 0.1.6h3 zstd pack fix [05.02.2022]
- Проверка, что запакованные данные могут занимать больше места
...
- Мета может генериться из структуры или не генериться вовсе для этого случая
...
- Логирование файла конфигурации

## 0.1.6d15 PacketBuilder Dlog [10.07.2021]
- Добавлен новый флаг для логирования приимаемых и передаваемых данных

## 0.1.6d14 RC_BOLD [07.07.2021]
- Новый макрос для лога, преобразование текста в логе

## 0.1.6d13 Examples fix [30.06.2021]
...
## 0.1.6d6 DWLOG [24.05.2021]
- Новый лог для отладки

## 0.1.6d5 abortTransaction [23.05.2021]
- Новая функция для БД - отмена транзакции

## 0.1.6d4 UI_CREATE_TOOLBAR_IN_WIDGET [21.05.2021]
- Макрос с параметрами для создания тулбара в переданном виджете

## 0.1.6d3 ItemSet::addItem [18.05.2021]
- Создание объекта из структуры данных возращает указатель на созданный объект

## 0.1.6d2 DB Column changes2 [17.05.2021]
- Добавлен нижний регистр для полей внешних ключей и поиска полей в таблицах
