...
void copyData(const Description& meta, const Data& data1, Data& data2)
{
	for (const Property& pr: meta.properties)
	{
		...
		case Ramio::Meta::Type::RecordPrtList:
			...
			break;
		case Ramio::Meta::Type::PKeyList:
			CAST_FIELDREL_BASE(data2, RMPKeyList, pr.diff,) = CAST_FIELDREL_BASE(data1, RMPKeyList, pr.diff, const); break;
		case Ramio::Meta::Type::TypeList:
			CAST_FIELDREL_BASE(data2, RMTypeList, pr.diff,) = CAST_FIELDREL_BASE(data1, RMTypeList, pr.diff, const); break;
